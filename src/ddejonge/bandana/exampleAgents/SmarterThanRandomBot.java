...
public class SmarterThanRandomBot extends Player{
    ...
    public static void main(String[] args){
        ...
        SmarterThanRandomBot bot = new SmarterThanRandomBot(name, finalYear, DEFAULT_GAME_SERVER_PORT);
    ...
    //CONSTRUCTOR.
    SmarterThanRandomBot(String name, int finalYear, int gameServerPort){
        this.name = "RandomBot";
...
//        }
    ...
    /**
     * Generates smarter order's list of orders for Spring or Fal phases.<br/>
     *
     * @return A list containing exactly one order for each of our units.
     */
    private List<Order> generateSmartMoveOrders() {
        ...
        //list of destinations we ordered to move to
        //this will help us to avoid conflict and replace them with support orders
        HashMap<String, Order> regionsWeMoveTo = new HashMap<>();
        ...
        for (Region unit : units) {
            ...
            // get the province of the region
            Province currentProvince = unit.getProvince();

            // check if the province is a supply center which is not owned by us
            if (currentProvince.isSC() && this.game.getOwner(currentProvince) != this.me) {
                currentOrder = new HLDOrder(this.me, unit);
            }

            // if current province is not a supple center
            else{

                List<Region> adjacentRegions = new ArrayList<>(unit.getAdjacentRegions());
                boolean foundAdjacentSC = false;

                for (Region r: adjacentRegions){
                    //check if the adjacent province is a SC that is not owned by us
                    //if so move there or support another unit that was already ordered to move there
                    if (r.getProvince().isSC() && this.game.getOwner(r.getProvince()) != this.me){

                        Order moveRegionOrder = regionsWeMoveTo.get(r.getName());
                        if (moveRegionOrder != null){
                            //we already have order to move to this region
                            //we will support this move instead of moving to this region
                            currentOrder = new SUPMTOOrder(me, unit, (MTOOrder) moveRegionOrder);
                            foundAdjacentSC = true;
                            break;

                        }
                        else{
                            // we didn't order yet to move to this region, we will do it now
                            currentOrder = new MTOOrder(me, unit, r);
                            regionsWeMoveTo.put(unit.getName(), currentOrder);
                            foundAdjacentSC = true;
                            break;
                        }
                    }
                ...
                if (!foundAdjacentSC){
                    // if we didn't find any adjacent supply center we just make random move
                    // including possibility to stay and hold in place
                    List<Region> potentialDestinations = new ArrayList<>(adjacentRegions);
                    ...
                    // if destination is current region - just hold
                    if(unit.equals(destination)){
                        currentOrder = new HLDOrder(me, unit);
                    }

                    // else - just move there
                    else {
                        currentOrder = new MTOOrder(this.me, unit, destination);
                        regionsWeMoveTo.put(unit.getName(), currentOrder);
                    }
                }
            }
